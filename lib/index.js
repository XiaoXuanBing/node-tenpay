...
class Payment {
  constructor({appid, mchid, partnerKey, pfx, APIv3Key, notify_url, refund_url, spbill_create_ip}) {
    ...
    this.refundKey = util.md5(partnerKey).toLowerCase();  // 退款数据解密key
    this.pfx = pfx;
  ...
  middlewareForExpress(type = 'pay') {
    return async (req, res, next) => {
      res.reply = msg => {
        let pkg = msg ? {return_code: 'FAIL', msg} : {return_code: 'SUCCESS'};
      ...
      let json;
      try {
        json = await util.parseXML(req.body);
      } catch (err) {
        return res.reply('ERR_XML');
      }
      ...
      if (type === 'pay') {
        if (json.sign !== this._getSign(json, json.sign_type)) res.reply('SIGN_INVALIED');
      }
